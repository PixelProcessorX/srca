...
--
-- Current Database: `rcdb`
--

CREATE DATABASE /*!32312 IF NOT EXISTS*/ `rcdb` /*!40100 DEFAULT CHARACTER SET utf8 COLLATE utf8_unicode_ci */;
...
CREATE TABLE `cats_event` (
  ...
  KEY `id_fit` (`id_fit`),
  CONSTRAINT `id_fitness2` FOREIGN KEY (`id_fit`) REFERENCES `cats_fit` (`id`) ON DELETE CASCADE ON UPDATE CASCADE
...
CREATE TABLE `cats_impr` (
  `id` int(11) NOT NULL AUTO_INCREMENT,
  `id_fit` int(11) NOT NULL DEFAULT '0',
  ...
  PRIMARY KEY (`id`),
  UNIQUE KEY `id_UNIQUE` (`id`),
  KEY `id_fitness_idx` (`id_fit`),
  CONSTRAINT `id_fitness` FOREIGN KEY (`id_fit`) REFERENCES `cats_fit` (`id`) ON DELETE CASCADE ON UPDATE CASCADE
...
CREATE TABLE `events_` (
  ...
  `WhenBeg` datetime DEFAULT NULL,
  `WhenEnd` datetime DEFAULT NULL,
  `WhenDaysHours` varchar(256) COLLATE utf8_unicode_ci DEFAULT 'Not Defined',
  `CostCents` int(11) DEFAULT '0',
  PRIMARY KEY (`id`),
  UNIQUE KEY `id_UNIQUE` (`id`),
  KEY `id_cat` (`id_cat`),
  KEY `id_users2_idx` (`id_instructor`),
  CONSTRAINT `id_eventcats` FOREIGN KEY (`id_cat`) REFERENCES `cats_event` (`id`) ON DELETE NO ACTION ON UPDATE NO ACTION,
  CONSTRAINT `id_users2` FOREIGN KEY (`id_instructor`) REFERENCES `users` (`id`) ON DELETE NO ACTION ON UPDATE NO ACTION
...
INSERT INTO `events_` VALUES (1,1,NULL,'Event #0','Junk data...',NULL,NULL,'Not Defined',123),(2,5,NULL,'Event #1','Junk data dfghbdrst...',NULL,NULL,'Not Defined',234),(3,3,NULL,'Event #2','adryhsxtfhbta...',NULL,NULL,'Not Defined',345),(4,1,NULL,'Event #3','Juszdrtbyzdthbfghfgha...',NULL,NULL,'Not Defined',456),(5,2,NULL,'Event #4','Aced Acrobatics...',NULL,NULL,'Not Defined',777),(6,6,NULL,'Event #5','74785675667...',NULL,NULL,'Not Defined',369),(7,1,NULL,'Event #6','Testingonetwothree...',NULL,NULL,'Not Defined',1111),(8,4,NULL,'Event #7','BigExcercise...',NULL,NULL,'Not Defined',4444),(9,1,NULL,'Event #8','Pascal\'s Plethora...',NULL,NULL,'Not Defined',12421),(10,3,NULL,'Event #9','Failed Doomsday',NULL,NULL,'Not Defined',2012);
...
CREATE TABLE `userevents` (
  ...
  `NotifyWhen` datetime DEFAULT NULL,
  PRIMARY KEY (`id`),
  UNIQUE KEY `id_UNIQUE` (`id`),
  KEY `id_events_idx` (`id_event`),
  KEY `id_users_idx` (`id_user`),
  CONSTRAINT `id_events` FOREIGN KEY (`id_event`) REFERENCES `events_` (`id`) ON DELETE CASCADE ON UPDATE CASCADE,
...
INSERT INTO `userevents` VALUES (1,5,8,'','\0','Quartz Task #0','\0',NULL),(2,5,9,'','\0','Quartz Task #1','\0',NULL),(3,5,5,'','\0','Quartz Task #2','\0',NULL),(4,3,7,'','\0','myEventNofification','\0',NULL),(5,2,2,'','\0','Yoga Class','\0',NULL),(6,1,1,'','\0','DUMBELLS','\0',NULL);
...
CREATE TABLE `users` (
  ...
  `GoogleID` varbinary(4096) DEFAULT NULL,
  `PassHash` varbinary(4096) DEFAULT NULL,
  `AuthToken` varbinary(4096) DEFAULT NULL,
  `Username` varchar(45) CHARACTER SET utf8 DEFAULT 'Anonymous User',
  `LoginAuthDB` varbinary(4096) NOT NULL,
  PRIMARY KEY (`id`),
  UNIQUE KEY `id_UNIQUE` (`id`)
) ENGINE=InnoDB DEFAULT CHARSET=utf8 COLLATE=utf8_unicode_ci COMMENT='The users in the database. Contains a minimum of information required to authenticate.';
/*!40101 SET character_set_client = @saved_cs_client */;
...
INSERT INTO `users` VALUES (1,'\0','\0',NULL,NULL,NULL,'Jhon Doe','08555442f6c28fee5cd214dbcc4254ea'),(2,'\0','\0',NULL,NULL,NULL,'Jane Doe','649e2632f53776e9b4dfd2c91e737189'),(3,'\0','\0',NULL,NULL,NULL,'Dr. Deqan Delinquist','7314000bbbdff4cf9f8e77d39d18af1e'),(4,'\0','\0',NULL,NULL,NULL,'Fake Oracle America Rep.','0a0ac6112d4e70ec9b8d5c78c7a0a562'),(5,'\0','\0',NULL,NULL,NULL,'Quartztet Qulaervich','70948cbf51f8b4b592924c382c705816');
...
SELECT @pAUTH:=LoginAuthDB FROM users WHERE users.id = user_id;
...
if(p_cat_fit_id = -1) then set p_cat_impr_id = NULL; END IF;
...
SELECT 'cats_event.*' FROM cats_event, cats_fit, cats_impr WHERE 
	...
	(p_name_contains IS NULL OR cats_fit.`Name` LIKE CONCAT('%',p_name_contains,'%')) AND
	(p_desc_contains IS NULL OR cats_fit.`Desc` LIKE CONCAT('%',p_desc_contains,'%'));
END ;;
DELIMITER ;
/*!50003 SET sql_mode              = @saved_sql_mode */ ;
/*!50003 SET character_set_client  = @saved_cs_client */ ;
...
if(p_name_contains = "NULL") then set p_name_contains = NULL; END IF;
if(p_desc_contains = "NULL") then set p_desc_contains = NULL; END IF;
SELECT 'cats_fit.*' FROM cats_fit, cats_impr WHERE 
...
if(p_name_contains = "NULL") then set p_name_contains = NULL; END IF;
if(p_desc_contains = "NULL") then set p_desc_contains = NULL; END IF;
SELECT 'cats_impr.*' FROM cats_impr WHERE 
	(p_cat_id IS NULL OR p_cat_id = cats_impr.id) AND 
	(p_name_contains IS NULL OR cats_impr.`Name` LIKE CONCAT('%',p_name_contains,'%')) AND
	(p_desc_contains IS NULL OR cats_impr.`Desc` LIKE CONCAT('%',p_desc_contains,'%'));
END ;;
DELIMITER ;
/*!50003 SET sql_mode              = @saved_sql_mode */ ;
/*!50003 SET character_set_client  = @saved_cs_client */ ;
...
BEGIN
SELECT 'events_.*' FROM _events WHERE true;
END ;;
DELIMITER ;
/*!50003 SET sql_mode              = @saved_sql_mode */ ;
/*!50003 SET character_set_client  = @saved_cs_client */ ;
...
SELECT 'events_.*' FROM events_, cats_event, cats_fit, cats_impr WHERE 
	(p_category IS NULL OR cats_event.id = p_category) AND 
	(p_instructor IS NULL OR events_.id_instructor =  p_instructor) AND 
	(p_name_contains IS NULL OR events_.`Name` LIKE CONCAT('%',p_name_contains,'%')) AND 
	(p_whenbeg_min IS NULL OR events_.WhenBeg >= p_whenbeg_min) AND 
	(p_whenbeg_max IS NULL OR events_.WhenBeg <= p_whenbeg_max) AND 
	(p_whenend_min IS NULL OR events_.WhenEnd >= p_whenend_min) AND 
	(p_whenend_max IS NULL OR events_.WhenEnd <= p_whenend_max) AND 
...
IN login_token VARCHAR(4096))
BEGIN
SELECT @pAUTH:=LoginAuthDB FROM users WHERE users.id = p_user_id;
IF pAUTH = login_token THEN 
	if(p_user_id = -1) then set p_user_id = NULL; END IF;
	SELECT 'userevents.*' FROM users INNER JOIN events_ WHERE (p_user_id IS NULL OR p_user_id = users.id);
END IF;
END ;;
DELIMITER ;
/*!50003 SET sql_mode              = @saved_sql_mode */ ;
...
/*!50003 SET sql_mode              = 'STRICT_TRANS_TABLES,NO_AUTO_CREATE_USER,NO_ENGINE_SUBSTITUTION' */ ;
DELIMITER ;;
CREATE DEFINER=`root`@`localhost` PROCEDURE `LoginFunction`(IN credentials VARBINARY(4096))
BEGIN
...
IN p5 VARCHAR(45))
BEGIN
SELECT CONCAT(MD5(RAND()),MD5(RAND()),MD5(RAND()),MD5(RAND()),
			  ...
			  MD5(RAND()),MD5(RAND()),MD5(RAND()),MD5(RAND()),
			  MD5(RAND()),MD5(RAND()),MD5(RAND()),MD5(RAND()),
			  MD5(RAND()),MD5(RAND()),MD5(RAND()),MD5(RAND())) AS login_token;
INSERT INTO users (
users.IsStaffMember, 
users.IsInstructor, 
users.GoogleID, 
...
users.Username,
users.LoginAuthDB) 
VALUES (0, 0, p2, p3, p4, p5, login_token);
END ;;
DELIMITER ;
/*!50003 SET sql_mode              = @saved_sql_mode */ ;
/*!50003 SET character_set_client  = @saved_cs_client */ ;
...
# what the randomly generated security token for the user is. Only the logged in user can 
# read or know it, and it cannot be accessed otherwise unless from an admin account.
SELECT @pAUTH:=LoginAuthDB FROM users WHERE users.id = p0;
IF pAUTH = login_token THEN 
...
-- Dump completed on 2016-03-11  9:55:35
